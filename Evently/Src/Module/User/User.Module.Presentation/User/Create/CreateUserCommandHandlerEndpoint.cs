...
internal sealed class CreateUserCommandHandlerEndpoint : IEndpoint
{
    ...
    {
        app.MapPost("/users", async (ILogger<UserRequestDTo> Logger, UserRequestDTo requestDTo, ISender sender, CancellationToken cancellationToken) =>
        {
            Logger.LogWarning("Creating user with data: {@UserData}", requestDTo);
